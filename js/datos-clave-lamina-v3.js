...
setText("airportName", tituloFinal);
    ...
    const ayudasHTML = [];
    if (radioayudas) ayudasHTML.push(`<span class="extra-kpi-meta">Radioayudas</span>${escapeHtml(radioayudas)}`);
    if (ayudasVisuales) ayudasHTML.push(`<span class="extra-kpi-meta">Ayudas visuales</span>${escapeHtml(ayudasVisuales)}`);
    setHTML("ayudasVisuales", ayudasHTML.length ? ayudasHTML.join("<br>") : "–");
