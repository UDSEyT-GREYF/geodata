...
function parseRutasOfertaCSV(text) {
  return parseCSV(text).map(r => {
    ...
    return {
      ...
      // Campos opcionales para discriminar sentido. En la fuente agregada por citypair
      // pueden no existir; si aparecen como Origen IATA / Destino IATA / Tipo de Movimiento
      // se usan para construir tablas separadas de arribos y partidas.
      origenIata: clean(firstNonEmpty(r, [
        "origen_iata",
        "iata_origen",
        "origen",
        "origin_iata",
      ...
      destinoIata: clean(firstNonEmpty(r, [
        "destino_iata",
        "iata_destino",
        "destino",
        "destination_iata",
      ...
      tipoMovimiento: clean(firstNonEmpty(r, [
        "tipo_de_movimiento",
        "tipo_movimiento",
        "movimiento_tipo",
        "tipo_mov",
...
/* ============================================================
   PÁGINA ANALÍTICA ADICIONAL
   Temporada alta/baja + tablas descargables por ruta/aerolínea
   ============================================================ */
...
function odGetMonthNumberFromKey(anioMes) {
  const d = parseFechaFlexible(anioMes);
...
function odBuildAdvancedSeasonality(summary) {
  const rows = (summary?.monthly || [])
    ...
    .sort((a, b) => clean(a.anioMes).localeCompare(clean(b.anioMes)));
  ...
  const byPaxDesc = rows.slice().sort((a, b) => b.pax - a.pax);
  const byPaxAsc = rows.slice().sort((a, b) => a.pax - b.pax);

  const high = byPaxDesc.slice(0, Math.min(3, byPaxDesc.length));
  const low = byPaxAsc.slice(0, Math.min(3, byPaxAsc.length));

  const totalPax = rows.reduce((acc, row) => acc + row.pax, 0);
  const highPax = high.reduce((acc, row) => acc + row.pax, 0);
  const lowPax = low.reduce((acc, row) => acc + row.pax, 0);
...
function odRenderSeasonalityAdvanced(iata, summary) {
  ...
  setHTML("odAdvancedSeasonalityText", data.note || "Sin datos suficientes.");

  setText("odHighSeasonMonths", data.high.length ? odJoinList(data.high.map(r => r.mes)) : "–");
  setText("odLowSeasonMonths", data.low.length ? odJoinList(data.low.map(r => r.mes)) : "–");

  setText(
    "odHighSeasonPax",
    data.high.length ? `${formatNumber(Math.round(data.high.reduce((acc, r) => acc + r.pax, 0)))} pax` : "–"
  );

  setText(
    "odLowSeasonPax",
    data.low.length ? `${formatNumber(Math.round(data.low.reduce((acc, r) => acc + r.pax, 0)))} pax` : "–"
  );

  setText(
    "odHighSeasonShare",
    Number.isFinite(data.highShare) ? odFormatPctPlain(data.highShare) : "–"
  );

  setText(
    "odLowSeasonShare",
    Number.isFinite(data.lowShare) ? odFormatPctPlain(data.lowShare) : "–"
  );
...
function odInferDirection(row, selectedIata) {
  const selected = clean(selectedIata).toUpperCase();
  const origen = clean(row?.origenIata).toUpperCase();
  const destino = clean(row?.destinoIata).toUpperCase();

  if (origen || destino) {
    if (destino === selected && origen !== selected) return "Arribos";
    if (origen === selected && destino !== selected) return "Partidas";
    if (origen === selected && destino === selected) return "Operación local";
    return "Otro sentido";
  }

  const tipo = normalizeTextKey(row?.tipoMovimiento);
  if (tipo.includes("arribo") || tipo.includes("aterriz")) return "Arribos";
  if (tipo.includes("partida") || tipo.includes("despeg")) return "Partidas";

  return "No discriminado";
}

function odHasDirectionalSourceData(rows) {
  return (rows || []).some(row => {
    const direction = clean(row.direction);
    return direction && direction !== "No discriminado" && direction !== "Otro sentido";
  });
}

function odBuildRouteAnalyticRowFromOfferRow(r, selected) {
  const otherCodeRaw = r.endpointA === selected ? r.endpointB : r.endpointA;
  if (!otherCodeRaw || otherCodeRaw === selected) return null;

  const otherMeta = getRouteMeta(otherCodeRaw);
  const otherNormalizedCode = clean(otherMeta?.iata || otherCodeRaw).toUpperCase();
  const destinationCode = getEquivalentDestinationCode(selected, otherNormalizedCode);
  if (!destinationCode || destinationCode === selected) return null;

  const isInternational = normalizeTextKey(r.clasificacion).includes("internacional");
  const label = getDestinationLabel(destinationCode, isInternational);
  const airportCodesLabel = otherNormalizedCode || destinationCode;
  const destinationName = clean(label.ciudad) || destinationCode;
  const market = isInternational ? "Internacional" : "Cabotaje";

  const route = `${getAirportBaseRouteName(selected)} - ${destinationName}${airportCodesLabel ? ` ${airportCodesLabel}` : ""}`;
  const pax = Number.isFinite(Number(r.pax)) ? Number(r.pax) : 0;
  const asientos = Number.isFinite(Number(r.asientos)) ? Number(r.asientos) : 0;
  const vuelos = Number.isFinite(Number(r.vuelos)) ? Number(r.vuelos) : 0;

  return {
    anioMes: clean(r.anioMes),
    mes: odFormatMonthLabelFromKey(r.anioMes),
    route,
    destinationCode,
    destinationName,
    market,
    airline: getAirlineDisplayName(r.airline),
    pax,
    asientos,
    vuelos,
    ocupacion: asientos > 0 ? pax / asientos : null,
    direction: odInferDirection(r, selected),
    origenIata: clean(r.origenIata).toUpperCase(),
    destinoIata: clean(r.destinoIata).toUpperCase()
  };
}

function odBuildFdoAnalyticRows(year = YEAR_REF) {
  const airline = "Aviación general / privada";
  return (fdoRoutesMonthlyAA || [])
    .filter(r => Number(r.y) === Number(year))
    .filter(r => (Number(r.p) || 0) > 0 || (Number(r.v) || 0) > 0)
    .map(r => {
      const label = getFdoRouteLabel(r.d);
      const monthKey = r.am || odMakeMonthKey(Number(r.y), Number(r.m));
      const pax = Number(r.p || 0);
      const vuelos = Number(r.v || 0);
      const asientos = Number(r.s || 0);
      const route = `${getAirportBaseRouteName("FDO")} - ${label.ciudad}${label.code ? ` ${label.code}` : ""}`;

      return {
        anioMes: monthKey,
        mes: odFormatMonthLabelFromKey(monthKey),
        route,
        destinationCode: label.code,
        destinationName: label.ciudad,
        market: label.clasificacion,
        airline,
        pax,
        asientos,
        vuelos,
        ocupacion: asientos > 0 ? pax / asientos : null,
        direction: "No discriminado",
        origenIata: "FDO",
        destinoIata: clean(label.code).toUpperCase()
      };
    })
    .sort((a, b) => clean(a.anioMes).localeCompare(clean(b.anioMes)) || clean(a.route).localeCompare(clean(b.route), "es"));
}

function odBuildRouteAnalyticRows(iata, year = YEAR_REF) {
  const selected = clean(iata).toUpperCase();

  if (isFDO(selected) && fdoRoutesMonthlyAA.length) {
  ...
  return rows
    ...
    .filter(Boolean)
    .filter(r => r.pax > 0 || r.asientos > 0 || r.vuelos > 0)
    .sort((a, b) =>
      clean(a.anioMes).localeCompare(clean(b.anioMes)) ||
      clean(a.route).localeCompare(clean(b.route), "es") ||
      clean(a.airline).localeCompare(clean(b.airline), "es")
    );
}

function odAggregateAnalyticRows(rows, groupKeys) {
  const acc = new Map();

  (rows || []).forEach(row => {
    const key = groupKeys.map(k => clean(row[k])).join("|");

    if (!acc.has(key)) {
      const item = {};
      groupKeys.forEach(k => { item[k] = row[k]; });
      item.pax = 0;
      item.asientos = 0;
      item.vuelos = 0;
      acc.set(key, item);
  ...
  return Array.from(acc.values())
    .map(row => ({
      ...row,
      ocupacion: row.asientos > 0 ? row.pax / row.asientos : null
    }))
    .sort((a, b) =>
      clean(a.anioMes).localeCompare(clean(b.anioMes)) ||
      clean(a.route).localeCompare(clean(b.route), "es") ||
      clean(a.airline || "").localeCompare(clean(b.airline || ""), "es") ||
      clean(a.direction || "").localeCompare(clean(b.direction || ""), "es")
    );
}
...
function odSafeFilePart(value) {
  return clean(value).toUpperCase().replace(/[^A-Z0-9_-]+/g, "_") || "AEROPUERTO";
}

function odRenderTable(tbodyId, rows, columns, emptyText = "Sin datos disponibles.") {
  const tbody = q(tbodyId);
  if (!tbody) return;

  if (!rows || !rows.length) {
    tbody.innerHTML = `<tr><td colspan="${columns.length}">${escapeHtml(emptyText)}</td></tr>`;
    return;
  }
...
function odRenderAdvancedTables(iata) {
  ...
  const routeRows = odAggregateAnalyticRows(rows, ["anioMes", "mes", "route", "market"]);
  const airlineRows = odAggregateAnalyticRows(rows, ["anioMes", "mes", "route", "market", "airline"]);

  const hasDirection = odHasDirectionalSourceData(rows);
  const directionRows = hasDirection
    ? odAggregateAnalyticRows(rows.filter(r => r.direction !== "No discriminado"), ["anioMes", "mes", "direction", "route", "market", "airline"])
    : [];

  const totalRowsEl = q("odAnalyticRowsCount");
  if (totalRowsEl) {
    totalRowsEl.textContent = `${formatNumber(rows.length)} registros base · ${formatNumber(routeRows.length)} filas ruta-mes · ${formatNumber(airlineRows.length)} filas ruta-mes-aerolínea`;
  }
  ...
  odSetDownloadButton(
    ...
    `oferta_demanda_ruta_aerolinea_mensual_${filePart}_${YEAR_REF}.csv`,
    airlineRows,
    [
      ...csvCommonCols.slice(0, 4),
      { key: "airline", label: "aerolinea" },
      ...csvCommonCols.slice(4)
    ]
  );
...
function renderAnaliticaAvanzada(iata, summary) {
  setText("odAdvancedAirportName", getAirportSheetTitle(odGetAirportRecordByIata(iata) || { IATA: iata }));
  setText("odAdvancedAirportName2", getAirportSheetTitle(odGetAirportRecordByIata(iata) || { IATA: iata }));

  odRenderSeasonalityAdvanced(iata, summary);
...
renderAnaliticaAvanzada(iata, summary);
