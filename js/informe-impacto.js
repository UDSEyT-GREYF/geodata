...
function extractRichTextSegments(node, inheritedBold = false) {
  ...
  if (!isElementNode) return segments;
...
function renderRichParagraph(pdf, paragraphEl, x, y, maxWidth, options = {}) {
  ...
  tokens.forEach(token => {
    ...
    if (!token.isSpace && lineUsed && (cursorX - x + tokenWidth > maxWidth)) {
      flushLine();
    }

    if (!token.isSpace || lineUsed) {
      pdf.text(txt, cursorX, cursorY);
      cursorX += tokenWidth;
      lineUsed = true;
    }
  });
...
async function addSummaryNativePage(pdf) {
  ...
  for (const p of paragraphEls) {
    const estimatedNextY = renderRichParagraph(pdf, p, marginL, y, textW, {
      fontSize: 10,
      lineHeight: 4.5,
      color: [34, 49, 61]
    });

    if (estimatedNextY > maxTextBottom) break;
    y = estimatedNextY + 1.5;
  }
